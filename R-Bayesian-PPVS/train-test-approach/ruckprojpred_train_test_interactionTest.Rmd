...
```{r}
ruck_data_std_train_limited_set = ruck_data_std_train %>% select(soltrms_final,stx_score)


tic()
ref_fit2 <- brm(stx_score ~ (.)^2,
               family = gaussian(),
               data = ruck_data_std_train_limited_set,
               prior = set_prior(R2D2()), # set a shrinkage prior, like horseshoe or R2D2
               backend = "cmdstanr",
               seed = 8675309,
               refresh = 0,
               ...
               control = list(adapt_delta = 0.99999))
toc()
pp_check(ref_fit2, type = "dens_overlay", ndraws = 100)
summary(ref_fit2)
bayesplot::mcmc_intervals(ref_fit2, pars = vars(starts_with("b_") & !ends_with("Intercept")))

```

```{r}
cvvs_valsearchF2 = projpred::cv_varsel(
  ref_fit,
  method = "forward",
  validate_search = FALSE,
  seed = 8675309
...
plot(cvvs_valsearchF2, stats = c('elpd', 'rmse', "mlpd"), deltas=TRUE, ranking_nterms_max = NA)
...
plot(cvvs2, stats = c('elpd', 'rmse', "mlpd"), deltas=TRUE, ranking_nterms_max = NA)

suggest_size(cvvs2) # this heuristic does not work here
modsize_decided2 = 2

soltrms2 <- solution_terms(cvvs2)
soltrms_final2 <- head(soltrms2, modsize_decided2)
soltrms_final2
```
...
best_fit2 <- brm(stx_score ~ doct_vel_ratio_whole + vel_ent_whole,
               family = "gaussian",
               data = ruck_data_std_train_limited_set,
...
ggplot(predictVsObs_train, aes(x = ...1, y = ...2)) +
  ...
  labs(title = "Bayesian Projection Predictive Variable Selection with LOO-CV (TRAIN)",
       subtitle = "stx_score ~ doct_vel_ratio_whole + vel_ent_whole",
