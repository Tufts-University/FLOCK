...
features = colnames(ruck_data[-ncol(ruck_data)])
for (f in features) {
  ruck_data <- poly_add_columns(ruck_data,!!rlang::sym(f),2)
}
load("ruckprojpred_stx_loo_2feat_no_contamination_2orderpolyEnv.RData") # uncomment when knitting
```
\newpage
